    ...
    if ('xls' == $Extension)
    {
        ...
        foreach ($sheetNames as $Index => $sheetName)
        {
            ...
            foreach ($Reader as $key => $row) {
                ...
                $sql = 'insert into onlinedata set ';
                foreach ($row as $k => $v) {
                    if (isset($onlineInsertArray[$k-1]) && $k > 0) {
                        $v = getTrim($v);
                        /*
                        $sqlkey = $onlineInsertArray[$k - 1];
                        if (isset($changeArray[$sqlkey])) {
                            $v = array_search($v, $changeArray[$sqlkey]);
                        }
                        */
                        $sql .= "{$onlineInsertArray[$k-1]} = '" . $v . "', ";
                    }
                }
                echo $sql."<br/>";
    ...
    elseif ('xlsx' == $Extension)
    {
        ...
        foreach ($sheetNames as $sheetName)
        {
            ...
            foreach ($data as $key => $row) {
                ...
                foreach($row as $k=>$v)
                {
                    if (isset($onlineInsertArray[$k-1]) && $k>0 && $k <= count($onlineInsertArray)+1)
                    {
                        $v = getTrim($v);
                        $sqlkey = $onlineInsertArray[$k-1];
                        $sql .= "{$onlineInsertArray[$k-1]} = '".$v."', ";
